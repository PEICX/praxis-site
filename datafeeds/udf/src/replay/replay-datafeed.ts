...
import {
	...
	getStepResolutionResumeMs,
	normalizeResolution,
	resolutionToSeconds,
	stepBarsMatchResolution,
} from './replay-bar-aggregate';
import { ReplayController } from './replay-controller';

type FullDatafeed = IExternalDatafeed & IDatafeedChartApi & Partial<IDatafeedQuotesApi>;

type ReplayPeriodParams = PeriodParams & { order?: 'asc' | 'desc' };
 ...
 */
export class ReplayDatafeedWrapper implements IExternalDatafeed, IDatafeedChartApi, Partial<IDatafeedQuotesApi> {
	private readonly _originalDatafeed: FullDatafeed;
	private readonly _replayController: ReplayController;
	
	private _isReplayMode: boolean = false;
	private _barSubscriptions: Map<string, BarSubscription> = new Map();
	private _unsubscribeTick: (() => void) | null = null;
	private _resetCacheCallbacks: Map<string, () => void> = new Map();
	private _liveSubscriptions: Set<string> = new Set();
	private _liveSubscriptionsPaused: boolean = false;
	private readonly _msThreshold: number = 1_000_000_000_000;
	private readonly _maxHistoryMs: number = 2 * 365 * 24 * 60 * 60 * 1000;
	private _replayStartTimeMs: number | null = null;
	private _historyLoadPromise: Promise<void> | null = null;
	private _forwardLoadPromise: Promise<boolean> | null = null;
	private _replayHistoryExhausted: boolean = false;
	private _replayForwardExhausted: boolean = false;
	private _chartResolution: ResolutionString | null = null;
	private _stepResolution: ResolutionString | null = null;
	private _symbolInfo: LibrarySymbolInfo | null = null;
	private _stepResolutionChangePromise: Promise<void> | null = null;

	constructor(originalDatafeed: FullDatafeed, replayController: ReplayController) {
	 ...
	 */
	public getReplayController(): ReplayController {
	...
	public async startReplay(
		symbolNameOrInfo: string | LibrarySymbolInfo,
		resolution: ResolutionString,
		startTime: number,
	 ...
	 */
	public async changeStepResolution(stepResolution: ResolutionString): Promise<void> {
		if (!this._isReplayMode || !this._symbolInfo || !this._chartResolution) {
			return;
		}

		const normalizedStep = normalizeResolution(stepResolution) as ResolutionString;
		...
		const run = async () => {
			...
			try {
				let bars = await this._loadStepBarsForResolutionChange(
					this._symbolInfo!,
					normalizedStep,
					resumeMs,
				);
				...
				if (newSeconds > currentSeconds) {
					bars = this._mergeCoarserStepBarsFromFinerMemory(
						finerBars,
						finerIndex,
						newSeconds,
						resumeMs,
				...
				bars = this._fillStepBarGapsFromFinerMemory(
					finerBars,
					newSeconds,
					resumeMs,
					bars,
				...
				if (!bars.length || !stepBarsMatchResolution(bars, newSeconds)) {
					console.warn(
						'[ReplayDatafeed] step resolution change: missing or mismatched bars',
						normalizedStep,
						'count=',
						bars.length,
					);
	 ...
	 */
	private _triggerResetCache(): void {
	...
	private async _handleReplayGetBars(
		symbolInfo: LibrarySymbolInfo,
		resolution: ResolutionString,
		periodParams: PeriodParams,
		onResult: HistoryCallback,
		...
		try {
			...
			if (meta.noData && visibleBars.length > 0) {
				let nextTimeMs: number | undefined;
				for (let i = visibleBars.length - 1; i >= 0; i--) {
	...
	private async _maybeLoadEarlierBars(
		symbolInfo: LibrarySymbolInfo,
		resolution: ResolutionString,
		fromMs: number,
		earliestVisibleMs: number,
		periodParams: PeriodParams
	): Promise<void> {
		...
		this._historyLoadPromise = this._requestHistoricalBars(
			symbolInfo,
			loadResolution,
			clampedFromMs,
			earliestVisibleMs,
	...
	private async _tryExtendForwardBars(): Promise<boolean> {
		...
		const beforeCount = this._replayController.getTotalBars();
		this._forwardLoadPromise = this._requestForwardBars(
			this._symbolInfo,
			this._stepResolution,
			lastBarTime,
			beforeCount,
		)
	...
	private async _requestForwardBars(
		symbolInfo: LibrarySymbolInfo,
		resolution: ResolutionString,
		...
		const periodParams: ReplayPeriodParams = {
			from: fromSec,
			to: toSec,
			countBack: 0,
		...
		console.log(
			'[ReplayDatafeed] requestForwardBars',
		...
		const bars = await new Promise<Bar[]>((resolve, reject) => {
			if ('getBars' in this._originalDatafeed && this._originalDatafeed.getBars) {
				this._originalDatafeed.getBars(
					symbolInfo,
					resolution,
					periodParams,
					(loaded: Bar[]) => {
	...
	private async _requestHistoricalBars(
		symbolInfo: LibrarySymbolInfo,
		resolution: ResolutionString,
		fromMs: number,
		toMs: number,
		...
		const requestParams: PeriodParams = {
	...
	public subscribeBars(
		...
		if (!this._isReplayMode && !this._liveSubscriptionsPaused) {
			...
			if ('subscribeBars' in this._originalDatafeed && this._originalDatafeed.subscribeBars) {
				this._originalDatafeed.subscribeBars(
					symbolInfo,
					resolution,
					onTick,
					listenerGuid,
	...
	private _pauseLiveSubscriptions(): void {
	...
	private _resumeLiveSubscriptions(): void {
		if (!this._liveSubscriptionsPaused) {
			return;
		}
		...
		if ('subscribeBars' in this._originalDatafeed && this._originalDatafeed.subscribeBars) {
			for (const listenerGuid of this._liveSubscriptions) {
				...
				try {
					this._originalDatafeed.subscribeBars(
						subscription.symbolInfo,
						subscription.resolution,
						subscription.callback,
						subscription.listenerGuid,
						this._resetCacheCallbacks.get(listenerGuid) || (() => undefined)
					);
				} catch (e) {
	...
	private async _resolveSymbol(symbolName: string): Promise<LibrarySymbolInfo> {
		return new Promise((resolve, reject) => {
			if ('resolveSymbol' in this._originalDatafeed && this._originalDatafeed.resolveSymbol) {
				this._originalDatafeed.resolveSymbol(
					symbolName,
					(symbolInfo: LibrarySymbolInfo) => {
						resolve(symbolInfo);
					},
	 ...
	 */
	private _mergeCoarserStepBarsFromFinerMemory(
		finerBars: Bar[],
		currentIndex: number,
		coarserSeconds: number,
		resumeMs: number,
		apiBars: Bar[],
	): Bar[] {
		if (!finerBars.length) {
		...
		const fromMemory = aggregateStepBarsToChartBars(
			finerBars,
			endIndex,
			coarserSeconds,
		).filter((bar) => bar.time >= resumeMs);
	...
	private _fillStepBarGapsFromFinerMemory(
		finerBars: Bar[],
		...
		const gapBuckets = aggregateStepBarsToChartBars(
			inGap,
			inGap.length - 1,
			targetStepSeconds,
		).filter((bar) => bar.time >= resumeMs && bar.time < gapEnd);
	 ...
	 */
	private async _loadStepBarsForResolutionChange(
		symbolInfo: LibrarySymbolInfo,
		stepResolution: ResolutionString,
		resumeMs: number,
	): Promise<Bar[]> {
		...
		console.log(
			'[ReplayDatafeed] loadStepBarsForResolutionChange',
			symbolInfo.ticker || symbolInfo.name,
			stepResolution,
			periodParams,
		);
	 ...
	 */
	private async _loadStepBarsAroundAnchor(
		symbolInfo: LibrarySymbolInfo,
		stepResolution: ResolutionString,
		anchorMs: number,
	): Promise<Bar[]> {
		...
		const forwardWindowSec = Math.min(
			forwardBars * secondsPerBar,
			maxForwardBars * secondsPerBar,
		);
		const fromSec = Math.max(0, anchorSec - preRollBars * secondsPerBar);
		const toSec = Math.min(
			nowSec,
			anchorSec + forwardWindowSec,
			Math.max(anchorSec, replaySec) + forwardWindowSec,
		);
		...
		console.log(
			'[ReplayDatafeed] loadStepBarsAroundAnchor',
			symbolInfo.ticker || symbolInfo.name,
			stepResolution,
			periodParams,
		);
	...
	private async _loadHistoricalBars(
		symbolInfo: LibrarySymbolInfo,
		resolution: ResolutionString,
		startTime?: number
	): Promise<Bar[]> {
	...
	private _estimateSecondsPerBar(resolution: ResolutionString): number {
	...
	private _normalizeBarsToMs(bars: Bar[]): Bar[] {
		if (!bars.length) {
			return bars;
		}
		let maxTime = 0;
	...
	private _shouldPushFormingBarUpdate(): boolean {
		...
		return (
			resolutionToSeconds(normalizeResolution(stepResolution)) <
			resolutionToSeconds(normalizeResolution(chartResolution))
		);
	}

	private _pushBarToSubscribers(): void {
